...
auto fileName = "/eos/user/i/ibozzaga/analysis_tcc/HiForestAOD_DATA_HBT_withMixing_500k.root"; //access tree here: https://www.dropbox.com/s/7yzuzy6cgpktg16/HiForestAOD_DATAtest_500k.root?dl=0
auto outFileName = "histos_mixing.root"; //output file saving the histograms, fit functions, etc...
...
TH1D * h_qinv_MixSS_OS = new TH1D("hqinvMixSS_OS","q_{inv} Mixing tracks",50,0,1);
TH1D * h_qinv_sig_SS_Ccorr = new TH1D("hqinvSigSS_Ccorr","q_{inv} SameEvt SS tracks Ccor",50,0,1); //aplying Coulomb correction
TH1D * h_qinv_MixSS_OS_Ccorr = new TH1D("hqinvMixSS_OS_Ccorr","q_{inv} Mixing tracks Ccor",50,0,1); //aplying Coulomb correction

std::ostringstream os; 
std::string folder_ ="SR_SSoverMix";
gSystem->Exec(Form("mkdir %s",folder_.c_str()));

std::string folder_Cent ="centCutMix";
gSystem->Exec(Form("mkdir %s",folder_Cent.c_str()));

std::string histName_qinvCentMix={"h_qinv_MixSS_OS"};
TString histTitle_h_qinvCentcut={"Centrality cut for mix"};

std::string histName_SR_SSoverMix={"h_qinv_sig_SS_Ccorr_clone"};
TString histTitle_h_sr_SSoverMix={"SR for Signal and Mix"};
...
t_1->Draw("qinvSigSS>>hqinvSigSS","HFsumET>900 && HFsumET<1210","goff"); //0-15% centrality
h_qinv_sig_SS->Write();
...
//35-60% -> 240-900GeV
//60-100% -> 0.0-240GeV
//it is possible to mix values, for example 35-100% -> 0-900 GeV
//centrality cut for opposite signs
t_1->Draw("qinvMixSS_OS>>hqinvMixSS_OS","HFsumETMixP1>900 && HFsumETMixP2>900 && HFsumETMixP1<1210 && HFsumETMixP2<1210");	  
h_qinv_MixSS_OS->Write();
c->Update();

c->SaveAs(Form("%s/%s.png",folder_Cent.c_str(),histName_qinvCentMix.c_str()));

t_1->Draw("qinvSigSS>>hqinvSigSS_Ccorr","coulombWSS*(HFsumET>900 &&  HFsumET<1210)","goff"); //0-15
h_qinv_sig_SS_Ccorr->Write();
c->Update();

t_1->Draw("qinvMixSS_OS>>hqinvMixSS_OS_Ccorr","coulombWOS*(HFsumETMixP1>900 && HFsumETMixP2>900 && HFsumETMixP1<1210 && HFsumETMixP2<1210)","goff");
h_qinv_MixSS_OS_Ccorr->Write();
c->Update();

TH1D * h_qinv_sig_SS_Ccorr_clone = (TH1D *)h_qinv_sig_SS_Ccorr->Clone("h_qinv_sig_SS_Ccorr_clone");
TH1D * h_qinv_MixSS_OS_Ccorr_clone = (TH1D *)h_qinv_MixSS_OS_Ccorr->Clone("h_qinv_MixSS_OS_Ccorr_clone");
...
Double_t int_den_controlRegion = h_qinv_MixSS_OS_Ccorr_clone->Integral(bin_for_normInt_min,bin_for_normInt_max);

h_qinv_MixSS_OS_Ccorr_clone->Scale(int_num_controlRegion/int_den_controlRegion);
h_qinv_sig_SS_Ccorr_clone->Divide(h_qinv_MixSS_OS_Ccorr_clone); //Single ratio: SS Divided by OS
...
h_qinv_sig_SS_Ccorr_clone->GetYaxis()->SetRangeUser(0.945,1.45);
...
//f_levy->SetParLimits(0,0.0,2.0);
   ...
   text2->DrawLatex(0.15, 0.82, "Centrality 30-35%");
   ...
   text->DrawLatex(0.15, 0.86, "CMS Open Data PbPb #sqrt{s_{NN}} = 2.76 TeV");
	...
	lgauss->Draw();
	//llevy->Draw();
    c->SaveAs(Form("%s/%s.png",folder_.c_str(),histName_SR_SSoverMix.c_str()));
